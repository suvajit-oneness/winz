...
@section('content')
    ...
    <div class="row">
        <div class="col-md-8 mx-auto">
            <div class="tile">
                <h3 class="tile-title">{{ 'Update Subject Category' }}
                    <span class="top-form-btn">
                        <a class="btn btn-secondary" href="{{ route('admin.subject.category.index') }}"><i class="fa fa-fw fa-lg fa-times-circle"></i>Cancel</a>
                    </span>
                </h3>
                <hr>
                <form action="{{ route('admin.subject.category.update') }}" method="POST" role="form" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="tile-body">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('categoryId') is-invalid @enderror" name="categoryId" id="categoryId">
                                ...
                                <option value="{{$item->id}}" {{($item->id == $sub_category->categoryId)? 'selected' : ''}}>{{$item->title}}</option>
                                @endforeach
                            </select>
