...
@section('content')
    <div class="fixed-row">
        <div class="app-title">
            ...
            <a href="{{ route('admin.questionpaper.create') }}" class="btn btn-primary pull-right"><i class="fa fa-fw fa-lg fa-plus"></i>Add New</a>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            $('#sampleTable').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.questionpaper.index') }}",
                columns: [
                    {data: 'board.name', name: 'board.name'},
                    {data: 'subject.name', name: 'subject.name'},
                    {data: 'class.name', name: 'class.name'},
                    {data: 'title', name: 'title'},
                    {data: 'id', name: 'question_papers.id',orderable: false, searchable: false,
                        render: function( data, type, full, meta ) {
                            if(full.image.length > 0){
                                var imageData = '<img style="width: 150px;height: 100px; display: block !important;" src="{{ asset('questionpaper/')}}/'+full.image+'">';
                                return imageData;    
                            }else{
                                return 'N/A';
                            }
                    ...
                    {data: 'id', name: 'question_papers.id',
                        render: function( data, type, full, meta ) {
                    ...
                    {data: 'id', name: 'question_papers.id',orderable: false, searchable: false,
                        render: function( data, type, full, meta ) {
                            var URL = "{{url('admin/questionpaper')}}/"+data+'/edit';
            ...
            $(document).on('click','.sa-remove',function(){
                ...
                function(isConfirm){
                  if (isConfirm) {
                    window.location.href = "questionpaper/"+languageId+"/delete";
                    } else {
            ...
            $(document).on('change','input[id="toggle-block"]',function(){
                ...
                $.ajax({
                    type:'POST',
                    dataType:'JSON',
                    url:"{{route('admin.questionpaper.updateStatus')}}",
