...
@section('content')
    <div class="fixed-row">
        <div class="app-title">
            ...
            <a href="{{ route('admin.board.create') }}" class="btn btn-primary pull-right"><i class="fa fa-fw fa-lg fa-plus"></i>Add New</a>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            $('#sampleTable').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.board.index') }}",
                columns: [
                    {data: 'name', name: 'name'},
                    {data: 'id', name: 'id',orderable: false, searchable: false,
                        render: function( data, type, full, meta ) {
                            if(full.image != null){
                                return '<img style="width: 150px;height: 100px;" src="{{URL::to('/').'/board/'}}'+full.image+'">';
                            }else{
                                return 'N/A';
                            }
                        }
                    ...
                    {data: 'id', name: 'id',orderable: false, searchable: false,
                        render: function( data, type, full, meta ) {
                            var URL = "{{url('admin/board')}}/"+data+'/edit';
            ...
            $(document).on('click','.sa-remove',function(){
                ...
                function(isConfirm){
                    if (isConfirm) {
                        window.location.href = "board/"+categoryId+"/delete";
                    } else {
            ...
            $(document).on('change','input[id="toggle-block"]',function(){
                ...
                $.ajax({
                    type:'POST',
                    dataType:'JSON',
                    url:"{{route('admin.board.updateStatus')}}",
